...
export async function handleWhatsAppMessage(sock, msg) {
  ...
  // Check if sender is temporarily suspended — delete their message
  if (isGroup && await isSuspended(jid, senderNumber)) {
    logger.info(`[SUSPEND] Deleting message from suspended member ${senderNumber} in ${jid}`);
    try {
      await sock.sendMessage(jid, { delete: msg.key });
    } catch (e) {
      logger.error(`[SUSPEND] Failed to delete suspended member message: ${e.message}`);
    }
    return;
  }
